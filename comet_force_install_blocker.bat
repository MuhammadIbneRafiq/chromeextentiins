@echo off
REM === Force-install Comet extension (prevents uninstall without admin) ===
...
echo Applying FORCE INSTALL policies for Comet extension %EXT_ID% ...
echo This will prevent uninstalling the extension without admin privileges.
echo.

REM 1) Ensure base key exists (Comet uses Chromium-based policy structure)
...
reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Perplexity\Comet" ^
 /v "IncognitoModeAvailability" /t REG_DWORD /d 1 /f

echo.
echo Done! Extension is now FORCE INSTALLED in Comet - cannot be removed without admin privileges.
