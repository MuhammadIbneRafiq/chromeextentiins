@echo off
REM === Force-install Brave extension (prevents uninstall without admin) ===
...
echo Applying FORCE INSTALL policies for Brave extension %EXT_ID% ...
...
REM 5) Force Private mode ON for this extension (incognito_mode = 2)
reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\BraveSoftware\Brave\ExtensionSettings\%EXT_ID%" ^
 /v "incognito_mode" /t REG_DWORD /d 2 /f

REM 6) Disable Private mode globally (TorDisabled and IncognitoModeAvailability)
reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\BraveSoftware\Brave" ^
 /v "IncognitoModeAvailability" /t REG_DWORD /d 1 /f

echo.
echo Done! Extension is now FORCE INSTALLED in Brave - cannot be removed without admin privileges.
